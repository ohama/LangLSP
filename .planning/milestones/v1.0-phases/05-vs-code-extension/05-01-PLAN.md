...
Create the icon directory and a 128x128 placeholder icon: `client/images/funlang-icon.png`.

Generate the icon using this deterministic Node.js script:
```bash
mkdir -p client/images
node -e "
const { createCanvas } = require('canvas');
// If canvas not available, create a minimal valid 128x128 PNG manually:
const fs = require('fs');
// Minimal valid 128x128 PNG (solid purple #7B2FBE square)
// Generated from a known-good minimal PNG template
const buf = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAIAAAACACAIAAABMXPacAAAAOklEQVR42u3BAQ0AAADCoPdPbQ43oAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA4GYvyAABgJqFDgAAAABJRU5ErkJggg==', 'base64');
fs.writeFileSync('client/images/funlang-icon.png', buf);
console.log('Icon created:', buf.length, 'bytes');
"
```

If the base64 approach fails for any reason, use ImageMagick as fallback:
```bash
convert -size 128x128 xc:'#7B2FBE' -fill white -gravity center -pointsize 48 -annotate 0 'FL' client/images/funlang-icon.png
```

If neither tool is available, create any valid PNG file at 128x128 or larger. The `vsce package` command requires the icon to exist but will accept any valid PNG.
