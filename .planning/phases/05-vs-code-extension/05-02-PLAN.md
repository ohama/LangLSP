...
CRITICAL: All existing test modules use the `[<Tests>]` attribute on their top-level test list for Expecto's `runTestsInAssembly` auto-discovery. The integration tests MUST also use `[<Tests>]`:
```fsharp
[<Tests>]
let tests =
