...
Purpose: Verify all three advanced features work correctly including edge cases like variable shadowing, non-renameable symbols, and diagnostic-driven quickfixes.
...
- "validates rename on variable": `let x = 1 in x` — prepare at x. Expect Some with placeholder = "x"
- "rejects rename on number literal": `42` — prepare at (0, 0). Expect None
- "rejects rename on keyword": `let x = 1 in x` — prepare at "let" keyword (0, 0). Expect None (Let node span covers "let x = 1 in x", but position 0,0 is "let" — hmm, this depends on what findNodeAtPosition returns. The Let node itself would be returned. prepareRename should handle Let as renameable. Adjust test expectation: Let IS renameable — the test should expect Some with the binding name.)
- Revised: "accepts rename on let binding": prepare at (0, 0) on `let x = 1 in x`. Since cursor is inside Let node, should return Some with placeholder "x"
...
<task type="auto">
  <name>Task 2: Create CodeActionsTests.fs, update fsproj, run all tests</name>
...
After updating fsproj, run:
```bash
dotnet test src/LangLSP.Tests/ --verbosity normal
```

All tests must pass, including the new ones AND all existing tests (no regressions from Diagnostics.fs changes — the added unused variable warnings should not break existing diagnostic tests unless they expected exact diagnostic counts).

CHECK for regressions: The Diagnostics.fs change adds unused variable warnings. Existing DiagnosticsTests.fs may test `analyze` and expect specific diagnostic counts. Review existing diagnostic test expectations:
- If tests check for empty diagnostics on valid code that happens to have unused variables, they will now get warnings
- Fix any broken tests by either:
  - Adjusting expected counts to include warnings
  - Using test code that doesn't have unused variables
  - Filtering by severity (only check Error diagnostics in existing tests)

If existing tests break, fix them in this task.
  </action>
  <verify>
```bash
dotnet test src/LangLSP.Tests/ --verbosity normal
# ALL tests pass (new + existing)
# Expected: ~90+ tests (79 existing + ~20 new)
```
  </verify>
  <done>
ReferencesTests.fs passes all reference tests (TEST-07). RenameTests.fs passes all rename tests (TEST-08). CodeActionsTests.fs passes all code action tests (TEST-09). No regressions in existing tests.
  </done>
</task>
...
# New test count: ~20 (References ~8, Rename ~6, CodeActions ~6)
# Total tests: ~99+
```
</verification>
...
- CodeActionsTests covers ACTION-01 (unused variable quickfix), unused detection accuracy
- All 3 test files follow project patterns (testSequenced, clearAll, Async.RunSynchronously)
- Zero test failures including existing tests
</success_criteria>
