...
이 문서는 F# LSP 서버 프로젝트를 처음부터 만드는 방법을 단계별로 설명한다. 이 튜토리얼을 따라하면 FunLang LSP 서버의 전체 프로젝트 구조를 생성할 수 있다.
...
프로젝트를 시작하기 전에 다음 도구들이 설치되어 있어야 한다.
...
생성된 `LangLSP.sln` 파일은 여러 F# 프로젝트를 하나로 묶어주는 역할을 한다.

### 2. 서버 프로젝트 생성

LSP 서버를 담을 F# 콘솔 프로젝트를 생성한다.
...
**왜 콘솔 프로젝트인가?**
- LSP 서버는 stdin/stdout을 통해 에디터와 통신한다
- 별도 프로세스로 실행되며, CLI 도구처럼 동작한다
...
솔루션에 추가하면 VS Code에서 전체 프로젝트를 한 번에 빌드하고 관리할 수 있다.

---

## NuGet 패키지 설치

LSP 서버에 필요한 NuGet 패키지들을 설치한다.
...
- **역할**: 디버깅용 구조화된 로깅
- **왜 파일 로깅인가?**: LSP 서버는 stdout을 프로토콜 통신에 사용하므로, 콘솔에 로그를 출력하면 프로토콜이 깨진다
...
FunLang 컴파일러 코드를 재사용하기 위해 Git 서브모듈로 추가한다.
...
FunLang 컴파일러는 이미 다음 기능들을 구현하고 있다.
...
LSP는 0-based Position을 사용하므로, Protocol.fs에서 변환 함수를 구현한다.
...
F#은 **파일 순서에 의존하는 언어**이다. F#에는 순방향 참조가 없으므로, 파일이 `.fsproj`에 나열된 순서대로 컴파일된다.
...
만약 `Server.fs`를 `Protocol.fs`보다 먼저 나열하면, 컴파일 에러가 발생한다.

### LangLSP.Server.fsproj 구조

프로젝트 파일의 전체 내용은 다음과 같다.
...
LSP 서버의 기능을 테스트하기 위한 테스트 프로젝트를 추가한다. (Phase 2에서 본격적으로 사용)
...
모든 설정이 완료되었으면, 프로젝트가 정상적으로 빌드되는지 확인한다.
...
프로젝트 설정이 완료되었다! 이제 다음 작업들을 진행할 수 있다.
