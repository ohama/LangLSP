...
이 문서는 Language Server Protocol(LSP)의 핵심 개념을 설명한다. LSP 서버를 구현하기 전에 반드시 이해해야 할 기초 개념들을 다룬다.
...
**Language Server Protocol(LSP)**은 프로그래밍 언어 지원 기능(자동 완성, 정의 찾기, 오류 진단 등)을 에디터/IDE와 언어 분석 엔진 사이에서 표준화한 프로토콜이다.

### 문제: 에디터 종속성

LSP 이전에는 각 에디터마다 각 언어에 대한 별도의 플러그인이 필요했다.
...
이 방식은 유지보수가 어렵고 중복 개발이 심했다.

### 해결책: LSP 표준화

LSP는 이 문제를 **M + N** 구조로 바꿨다.

```
M개 에디터 (LSP 클라이언트) + N개 언어 (LSP 서버) = M + N개 구현
```

각 에디터는 LSP 클라이언트만 구현하면 되고, 각 언어는 LSP 서버만 구현하면 된다. 그러면 모든 조합이 자동으로 작동한다.
...
클라이언트와 서버는 **별도 프로세스**로 실행되며, 다음 방법으로 통신한다.
...
대부분의 LSP 서버는 stdin/stdout을 사용한다. 에디터가 서버를 자식 프로세스로 실행하고, 표준 입출력을 통해 메시지를 주고받는다.
...
LSP는 **JSON-RPC 2.0** 프로토콜을 기반으로 한다.

### 메시지 형식

모든 LSP 메시지는 다음 형식을 따른다.
...
LSP는 3가지 메시지 타입을 사용한다.

#### 1. Request (요청)

클라이언트 또는 서버가 상대방에게 작업을 요청하고 응답을 기다린다.
...
응답을 기대하지 않는 단방향 메시지이다. `id` 필드가 없다.
...
모든 LSP 세션은 `initialize` 요청으로 시작된다.
...
LSP의 핵심 설계 원칙 중 하나는 **기능 협상**이다. 모든 서버가 모든 기능을 구현할 필요는 없다.

### 클라이언트 Capabilities

`initialize` 요청에서 클라이언트가 지원하는 기능을 알린다.
...
`initialize` 응답에서 서버가 제공하는 기능을 선언한다.
...
**중요**: 서버가 `hoverProvider: true`를 선언하지 않으면, 클라이언트는 `textDocument/hover` 요청을 보내지 않는다.

---

## Document Synchronization (문서 동기화)

에디터에서 파일을 열고 편집하면, 서버는 그 변경사항을 알아야 한다. 이를 **문서 동기화**라고 한다.

### Lifecycle 이벤트

#### textDocument/didOpen

사용자가 파일을 열면 클라이언트가 보내는 알림이다.
...
파일을 닫으면 보내는 알림이다. 서버는 메모리에서 해당 파일 정보를 제거할 수 있다.

### Sync 방식

서버는 두 가지 동기화 방식 중 하나를 선택할 수 있다.
...
**변경된 부분만** (range + text) 전송한다.

**장점**: 효율적, 네트워크 비용 감소
**단점**: 구현 복잡도 증가 (range 관리)

**권장**: 대부분의 프로덕션 서버는 Incremental Sync를 사용한다.

---

## 실시간 진단 (Real-time Diagnostics)

LSP의 가장 강력한 기능 중 하나는 **실시간 오류/경고 표시**이다.

### textDocument/publishDiagnostics

서버가 클라이언트에게 진단 정보를 보내는 알림이다. (Request가 아닌 **Notification**)
...
**중요**: `publishDiagnostics`는 서버가 능동적으로 보내는 알림이다. 클라이언트가 요청하지 않아도 서버가 변경 감지 시 자동으로 전송한다.
...
다음 문서에서는 F#으로 LSP 서버를 구현하기 위한 라이브러리 선택을 다룬다.
